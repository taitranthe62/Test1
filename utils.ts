...
/**
 * Implements a parsing algorithm to repair a JSON string that may contain unescaped backslashes
 * from LaTeX commands. This function iterates through the string, identifying string literals and
 * specifically targeting backslashes that are not part of valid JSON escape sequences (like \n, \", \\)
 * and doubling them to ensure they are correctly interpreted by JSON.parse().
 *
 * This addresses the "Unterminated string" error caused by AI-generated LaTeX.
 *
 * @param jsonString The raw, potentially malformed JSON string from the AI.
 * @returns A repaired JSON string ready for parsing.
 */
export const repairJsonWithLatex = (jsonString: string): string => {
    enum State {
        OutsideString,
        InString,
        InStringEscaped,
    ...
    for (const char of jsonString) {
        switch (state) {
            ...
            case State.InStringEscaped:
                // We are at the character immediately following a backslash inside a string.
                if ('"\\/bfnrt'.includes(char)) {
                    // This is a valid JSON escape sequence. The backslash was correct.
                    result += char;
                } else {
                    // This is not a standard JSON escape char (e.g., 's' in `\sum`, or '{' in `\{`).
                    // This is a LaTeX command that needs its backslash escaped.
                    // We go back one step and insert an extra backslash before the original one.
