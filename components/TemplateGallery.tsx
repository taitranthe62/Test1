    ...
    return (
      <div className="flex flex-col items-center">
        ...
        >
          ...
          >
              <SlideBackground background={{ color: '#f8f9fa', primaryTextColor: '#000', secondaryTextColor: '#333' }} />
              {template.previewElements.map((element, index) => {
                  const elementStyle: React.CSSProperties = {
                      ...
                      // Add generic styling for previews
                      backgroundColor: element.type === 'SHAPE' ? '#e0e0e0' : 'transparent',
                      color: element.type === 'TEXT' ? '#333' : (element.type === 'ICON' ? '#a0a0a0' : 'transparent'),
                      border: element.type === 'TEXT' ? '1px dashed #ccc' : 'none',
                  };
                  return (
                      <div key={index} style={elementStyle}>
                          {element.type === 'TEXT' && 'content' in element ? (
                              <div className="p-2" dangerouslySetInnerHTML={{ __html: element.content || '' }}></div>
                          ) : element.type === 'IMAGE' ? (
